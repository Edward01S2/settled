      ...
      @foreach($items as $item)
        ...
        <div class="item relative flex items-center text-center overflow-hidden {!! $item['align'] !!}">
          <div class="item-container relative z-30 px-6 py-12 md:w-1/2 md:py-20 lg:w-1/2 lg:px-16 xl:w-5/12">
            ...
            @if($item['title'])
              <h3 class="font-brandon text-white uppercase tracking-wider text-xl mb-2 lg:text-2xl">{!! $item['title'] !!}</h3>
            @endif

            <p class="text-white lg:text-lg xl:text-xl">{!! $item['content'] !!}</p>

            @if($item['link'])
              <div class="link-text font-brandon font-bold tracking-wider text-c-tan-100 uppercase mt-4 md:mt-8">{!! $item['link']['title'] !!}</div>
            @endif
          </div>
